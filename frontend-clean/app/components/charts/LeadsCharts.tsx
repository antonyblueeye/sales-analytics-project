...
interface LeadsChartsProps {
    titleData: {title: string, count: number}[];
    repliedTitleData?: {title: string, count: number}[];
    locationData: any[];
}

export default function LeadsCharts({ titleData, repliedTitleData = [], locationData }: LeadsChartsProps) {
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-[#1e293b]/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl shadow-xl p-6 text-slate-100">
                    <h2 className="text-xl font-bold mb-6 tracking-tight">Most Replied Titles</h2>
                    {repliedTitleData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                            <BarChart data={repliedTitleData} layout="vertical" margin={{ left: -20, right: 10 }}>
                                <XAxis type="number" hide />
                                <YAxis 
                                    type="category" 
                                    dataKey="title" 
                        ...
                        <div className="flex flex-col items-center justify-center h-[300px] text-slate-500">
                            <p className="text-sm font-medium uppercase tracking-widest text-slate-600">No Replies Yet</p>
